...
import { View, Text, Button, StyleSheet, Alert, ActivityIndicator, ScrollView, TextInput } from "react-native";
...
export default function Activity1() {
      ...
      getChapter1Activity1UserInput(user.uid).then((data: string[] | null) => {
        if (data) {
          ...
          if (data.includes("Other")) {
            // 如果需要处理 "Other" 的输入，例如从其他字段获取
            // 可以在此处设置 otherInput 的值
            // 例如，假设有一个对应的输入字段存储用户输入
            // 这里暂时保持空，可以根据实际需求修改
          }
        }
      });
  ...
  const handleSubmit = () => {
    ...
    if (checkboxes.includes("Other") && otherInput.trim() !== "") {
      selections = selections.map((item: string) => (item === "Other" ? otherInput.trim() : item));
    }

    updateChapter1Activity1(user.uid, selections)
      .then(() => {
