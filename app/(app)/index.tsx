...
async function updateAllChapterProgress(
  ...
  try {
    ...
    const currentData = currentProgress.exists()
      ? (currentProgress.data() as ChapterStatus)
      : defaultChapterStatus;
    ...
    for (let i = startIndex; i < chapters.length; i++) {
      ...
      if (chapterDoc.exists()) {
        ...
        const isFinished = Object.values(chapterData).every(
          (val) => val === "1"
        );
        const status = isFinished
          ? "2"
          : Object.values(chapterData).some((val) => val === "1")
          ? "1"
          : "0";
...
export default function HomePage() {
  ...
  const { chap1Percent, chap2Percent, chap3Percent, chap4Percent } =
    useChapterProgressContext(); //you can use this for the chapter progress
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      if (user) {
        ...
        const progressData = await updateAllChapterProgress(user.uid, [
          ...validchap,
        ]);
        if (progressData) {
          setStatuses(progressData);
          const completedChapters = validchap.filter(
            (chap) => progressData[chap] === "2"
          ).length;
      ...
      <View style={styles.bottomNav}>
        {[
          ...
          },
          {
            ...
            route: "/(app)/tracker" as const,
          },
          {
            name: "Achieve",
            icon: require("@/assets/images/achievement.png"),
            route: "/(app)/achieve" as const,
          },
          {
            name: "Profile",
