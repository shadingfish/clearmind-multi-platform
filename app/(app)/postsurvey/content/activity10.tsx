...
const Activity10 = () =>{
    ...
    useEffect(() => {
      if (user) {
        console.log('getPostsurveyActivity10')
        getPostsurveyActivity(user.uid, "Activity10")
          .then((snapshot) => {
            if (snapshot.exists()) {
          ...
          .catch((err) => console.log("Error get postsurvey activity10:", err));
      }
    }, [pending]);
    ...
    return (
        ...
        <ChapterNavigationButton
            prev={"/(app)/postsurvey/content/activity9"}
            next={() => {if (hasEmptyValues(data)) {
                toast.show("Empty Input");
            } else {
                setPostsurveyActivity(user!.uid, data, "Activity10");
                router.push("/(app)/postsurvey/content/activity11" as RelativePathString);
            }
            }}
        />
