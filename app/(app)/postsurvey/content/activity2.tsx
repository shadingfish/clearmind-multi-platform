...
const Activity2 = () => {
    ...
    useEffect(() => {
      if (user) {
        console.log('getPostsurveyActivity2')
        getPostsurveyActivity(user.uid, "Activity2")
          .then((snapshot) => {
            if (snapshot.exists()) {
          ...
          .catch((err) => console.log("Error get postsurvey activity2:", err));
      }
    }, [pending]);
    ...
    return (
        ...
        <ChapterNavigationButton
            prev={"/(app)/postsurvey/content/activity1"}
            next={() => {if (hasEmptyValues(data)) {
                toast.show("Empty Input");
            } else {
                setPostsurveyActivity(user!.uid, data, "Activity2");
                router.push("/(app)/postsurvey/content/activity3" as RelativePathString);
            }
            }}
        />
        </YStack>
