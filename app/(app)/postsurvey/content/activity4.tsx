...
import { useAuth } from "@/hooks/useAuth";
  ...
  const Activity4 = () =>{
    ...
    useEffect(() => {
      if (user) {
        console.log('getPostsurveyActivity4')
        getPostsurveyActivity(user.uid, "Activity4")
          .then((snapshot) => {
            if (snapshot.exists()) {
          ...
          .catch((err) => console.log("Error get postsurvey activity4:", err));
      }
    }, [pending]);
    ...
    return (
        ...
        <ChapterNavigationButton
            prev={"/(app)/postsurvey/content/activity3"}
            next={() => {if (hasEmptyValues(data)) {
                toast.show("Empty Input");
            } else {
                setPostsurveyActivity(user!.uid, data, "Activity4");
                router.push("/(app)/postsurvey/content/activity5" as RelativePathString);
            }
            }}
        />
