...
const Activity5 = () =>{
    ...
    useEffect(() => {
      if (user) {
        console.log('getPostsurveyActivity5')
        getPostsurveyActivity(user.uid, "Activity5")
          .then((snapshot) => {
            if (snapshot.exists()) {
          ...
          .catch((err) => console.log("Error get postsurvey activity5:", err));
      }
    }, [pending]);
    ...
    return (
        ...
        <ChapterNavigationButton
            prev={"/(app)/postsurvey/content/activity4"}
            next={() => {if (hasEmptyValues(data)) {
                toast.show("Empty Input");
            } else {
                setPostsurveyActivity(user!.uid, data, "Activity5");
                router.push("/(app)/postsurvey/content/activity6" as RelativePathString);
            }
            }}
        />
